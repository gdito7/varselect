...
pg_title <- "Automatic Variables Selection"
          ...
          fluidRow(
            column(width = 9,
                ...
                HTML('<div id="fwd_criteria" class="form-group shiny-input-radiogroup shiny-input-container" width="100%">
                        <div class="custom-controls-stacked shiny-options-group shiny-input-container">
                          <label class="custom-control custom-radio">
                            <input type="radio" class="custom-control-input" name="fwd_criteria" value="AICc"  checked="checked">
                            <span class="custom-control-label">AICc</span>
                          </label>
                          <label class="custom-control custom-radio">
                          ...
                          <label class="custom-control custom-radio">
                            ...
                            <span class="custom-control-label">Adjusted R-squared</span>
                          </label>
                        </div>
                    </div>'),
                width = 6
              ...
              tablerCard(
                ...
                h6("Selction Parameter (Lambda)"),
                HTML('<div id="lasso_criteria" class="form-group shiny-input-radiogroup shiny-input-container" width="100%">
                        <div class="custom-controls-stacked shiny-options-group shiny-input-container">
                          <label class="custom-control custom-radio">
                            <input type="radio" class="custom-control-input" name="lasso_criteria" value="RMSE"  checked="checked">
                            <span class="custom-control-label"> Cross-Validation (RMSE)</span>
                          </label>
                          <label class="custom-control custom-radio">
              ...
              tablerCard(
                ...
                HTML('<div id="ridge_criteria" class="form-group shiny-input-radiogroup shiny-input-container" width="100%">
                        <div class="custom-controls-stacked shiny-options-group shiny-input-container-inline">
                          <label class="custom-control custom-radio ">
                            <input type="radio" class="custom-control-input" name="ridge_criteria" value="RMSE"  checked="checked">
                            <span class="custom-control-label">Cross-Validation (RMSE)</span>
                          </label>
                          <label class="custom-control custom-radio">
              ...
              tablerCard(
                ...
                HTML('<div id="scad_criteria" class="form-group shiny-input-radiogroup shiny-input-container" width="100%">
                        <div class="custom-controls-stacked shiny-options-group shiny-input-container-inline">
                          <label class="custom-control custom-radio ">
                            <input type="radio" class="custom-control-input" name="scad_criteria" value="RMSE"  checked="checked">
                            <span class="custom-control-label">Cross-Validation (RMSE)</span>
                          </label>
                          <label class="custom-control custom-radio">
        ...
        tablerTabItem(
          ...
          fluidRow(
            ...
            column(width = 4,
                   tablerCard(width = 12,
                              status = "primary",
                              title = h4("Optimal Parameters"),
                              closable = FALSE,
                              zoomable = TRUE, 
                              # br(),
                              dataTableOutput("result_parameters")
                   )
            )
          )
        ),
  ...
  server = function(input, output, session) {
    ...
    dt_col_target <- eventReactive(input$submit_target, {
      ...
      if(is.null(input$data)) {
        ...
        xs$Role <- ifelse(xs$Variable == input$col_y, "Response", "Predictor")
      }
      xs
    })
    ...
    method_cr <- reactive({
      ...
      # if()
      crit=c(input$enable_forward, input$enable_lasso, input$enable_ridge,input$enable_scad)
      method <- c("Forward", "LASSO", "Ridge","SCAD")
      method <- method[crit]
      
      value <- c(NA, input$lasso_user_input, input$ridge_user_input,input$scad_user_input)
      value <- value[crit]
      
  
      criterion <- c(input$fwd_criteria, input$lasso_criteria, input$ridge_criteria,input$scad_criteria)
      criterion <- criterion[crit]
      
      criterion <- ifelse(criterion == 'User', paste0(criterion, "(", value, ")"),
                          ifelse(criterion == 'AICc'|criterion == 'BIC'|criterion == 'adjrsq',criterion,"CV")
      )
    ...
    observeEvent(input$processBtn, {
      ...
      if(is.null(input$data)){
        ...
        dt_lm <- reactive({
          ...
          response=dt_role()$Variable[dt_role()$Role == "Response"]
          fm=as.formula(paste(response,"~.",sep = ""))
          lm(fm,data = dt)
         # lm(dt[, dt_role()$Variable[dt_role()$Role == "Response"]] ~ ., data = dt[, names(dt) != dt_role()$Variable[dt_role()$Role == "Response"]])
        })
        
        dta_src=reactive({
          dta <- dt_target() #data.frame(weight, group)
          response=dt_role()$Variable[dt_role()$Role == "Response"]
          fm1=as.formula(paste(response,"~.",sep = ""))
          result=list("data"=dta,"response"=response,"formula"=fm1)
          return(result)
        })
        #browser()
        fwd_method=reactive(
          {
            dta=dta_src()$data
            response=dta_src()$response
            fm1=dta_src()$formula
            crit_step_num=input$fwd_criteria
            mod_step=stepwise(dta,response,
                              selection = "forward",select = crit_step_num)
            mod=lm(fm1,data = dta)
            coef_step=data.frame(names=names(mod$coefficients),
                                 value=mod$coefficients)
            rownames(coef_step)=NULL
            coef_step$status=as.numeric(coef_step$names%in%mod_step$variate)
            step_crit=data.frame(name=crit_step_num,
                                 value=tail(mod_step$process$Select,1))
            result=list("coef"=coef_step,"criteria"=step_crit)
          }
        )
        
        lasso_method=reactive(
          {
            
            dta=dta_src()$data
            response=dta_src()$response
            fm1=dta_src()$formula
            mod_lasso=cv.glmnet(fm1,data=dta,alpha=1)
            coef_lasso=data.frame(names=dimnames(coef(mod_lasso,s="lambda.min"))[[1]],
                                  value=as.numeric(coef(mod_lasso),s="lambda.min"))
            coef_lasso$status=ifelse(coef_lasso$value==0,0,1)
            lasso_crit=data.frame(name=c("lambda","RMSE"),
                                  value=c(mod_lasso$lambda.min,sqrt(min(mod_lasso$cvm))))
            
            result=list("coef"=coef_lasso,"criteria"=lasso_crit)
          }
        )

        lasso_method_user=reactive(
          {
            user_param=input$lasso_user_input
            dta=dta_src()$data
            response=dta_src()$response
            fm1=dta_src()$formula
            mod_lasso=glmnet(fm1,data=dta,alpha=1,lambda=user_param)
            coef_lasso=data.frame(names=dimnames(coef(mod_lasso))[[1]],
                                  value=as.numeric(coef(mod_lasso)))
            coef_lasso$status=ifelse(coef_lasso$value==0,0,1)
            response=model.response(model.frame(fm1,data = dta))
            res_hat=predict(mod_lasso,newdata=dta)
            error=response-res_hat
            rmse=sqrt(mean(error^2))
            
            lasso_crit=data.frame(name=c("lambda","RMSE"),
                                  value=c(user_param,rmse))
            
            
            result=list("coef"=coef_lasso,"criteria"=lasso_crit)
          }
        )
        ridge_method=reactive(
          {
            
            dta=dta_src()$data
            response=dta_src()$response
            fm1=dta_src()$formula
            mod_ridge=cv.glmnet(fm1,data=dta,alpha=0)
            coef_ridge=data.frame(names=dimnames(coef(mod_ridge,s="lambda.min"))[[1]],
                                  value=as.numeric(coef(mod_ridge,s="lambda.min")))
            quant=quantile(abs(coef_ridge$value),probs=0.75)
            coef_ridge$status=ifelse(coef_ridge$value<quant,0,1)
            ridge_crit=data.frame(name=c("lambda","RMSE"),
                                  value=c(mod_ridge$lambda.min,sqrt(min(mod_ridge$cvm))))
            result=list("coef"=coef_ridge,"criteria"=ridge_crit)
          }
        )
        
        ridge_method_user=reactive(
          {
            user_param=input$ridge_user_input
            dta=dta_src()$data
            response=dta_src()$response
            fm1=dta_src()$formula
            mod_ridge=glmnet(fm1,data=dta,alpha=0,lambda=user_param)
            coef_ridge=data.frame(names=dimnames(coef(mod_ridge))[[1]],
                                  value=as.numeric(coef(mod_ridge)))
            coef_ridge$status=ifelse(coef_ridge$value==0,0,1)
            response=model.response(model.frame(fm1,data = dta))
            res_hat=predict(mod_ridge,newdata=dta)
            error=response-res_hat
            rmse=sqrt(mean(error^2))
        ...
        scad_method=reactive({
          dta=dta_src()$data
          response=dta_src()$response
          fm1=dta_src()$formula
          y=model.response(model.frame(fm1,data = dta))
          X=model.matrix(fm1,data = dta)[,-1]
          mod_scad=cv.ncvreg(X,y,family="gaussian",penalty="SCAD")
          coef_scad=data.frame(names=names(coef(mod_scad)),value=as.numeric(coef(mod_scad)))
          coef_scad$status=ifelse(coef_scad$value==0,0,1)
          
          scad_crit=data.frame(name=c("lambda","RMSE"),
                               value=c(mod_scad$lambda.min,sqrt(min(mod_scad$cve))))
          result=list("coef"=coef_scad,"criteria"=scad_crit)
        })
        scad_method_user=reactive({
          user_param=input$scad_user_input
          dta=dta_src()$data
          response=dta_src()$response
          fm1=dta_src()$formula
          y=model.response(model.frame(fm1,data = dta))
          X=model.matrix(fm1,data = dta)[,-1]
          mod_scad=ncvreg(X,y,family="gaussian",penalty="SCAD",
                          lambda=user_param+0.000001)
          coef_scad=data.frame(names=names(coef(mod_scad)),value=as.numeric(coef(mod_scad)))
          coef_scad$status=ifelse(coef_scad$value==0,0,1)
          res_hat=predict(mod_scad,X)
          error=y-res_hat
        ...
        output$results <- renderDataTable({
          if(n > 0) {
             ...
             if(any(method_cr()[,"method"] %in% "Forward")){
               coef_step=fwd_method()$coef
              } 
            if(any(method_cr()[,"method"] %in% "LASSO")){
              if(input$lasso_criteria=="User"){
              coef_lasso=lasso_method_user()$coef
              }else{
                coef_lasso=lasso_method()$coef 
              }              
              } 
            if(any(method_cr()[,"method"] %in% "Ridge")){
              if(input$ridge_criteria=="User"){
                coef_ridge=ridge_method_user()$coef
              }else{
                coef_ridge=ridge_method()$coef 
              }
            }
              if(any(method_cr()[,"method"] %in% "SCAD")){
                if(input$scad_criteria=="User"){
                coef_scad=scad_method_user()$coef
                }else{
                  coef_scad=scad_method()$coef 
                }
              }
            ...
            response=dta_src()$response
            fm1=dta_src()$formula
            coef_method=c("coef_step","coef_lasso","coef_ridge","coef_scad")
            coef_crit=c(input$enable_forward, input$enable_lasso, input$enable_ridge,input$enable_scad)
            status_coef=lapply(coef_method[coef_crit],
                               function(coef){get(coef)[,"status"]})   
            coef_name=names(dta)[-which(names(dta)==response)]
            dtn=data.frame(do.call(cbind,status_coef))
            names(dtn)=c("Forward","LASSO","Ridge","SCAD")[coef_crit]
            dtn=dplyr::arrange_all(dtn,dplyr::desc)
            dtn=cbind("Variables"=coef_name,dtn[-1,])
            del_zero=rowSums(dtn[,-1])>0
            dtn=dtn[del_zero,]
            #total_dtn=c("Varibles"=NA,colSums(dtn[,-1]))
            #dtn=rbind(total_dtn,dtn)
            dtn$Variables=as.character(dtn$Variables)
            #dtn[1,1]="Total"
            assign("total_dtn",colSums(dtn[,-1]),envir = .GlobalEnv)
            datatable(dtn,
                      rownames = FALSE,
                      options = list(pageLength = 5,
                                     lengthMenu = c(5, 10),
        ...
        output$Total <- renderDataTable({
          dta_tot=as.data.frame(matrix(total_dtn,1,length(total_dtn)))
          dta_all=cbind("Number of Selected Parameter",dta_tot)
          coef_crit=c(input$enable_forward, input$enable_lasso, input$enable_ridge,input$enable_scad)
          colnames(dta_all)=c("",c("Forward","LASSO","Ridge","SCAD")[coef_crit])
          datatable(dta_all,
                    rownames = FALSE,
      ...
      output$result_criteria <- renderDataTable({
        ...
        if(n > 0) {
          isolate(method_cr())
          is_user=c(F,input$lasso_criteria=="User",
                    input$ridge_criteria=="User",
                    input$scad_criteria=="User")
          sel_method=c("fwd_method","lasso_method","ridge_method","scad_method")
          sel_method=sapply(seq_along(sel_method),function(i){
            if(is_user[i]){
            paste(sel_method[i],"_user",sep="")
            }else{
              sel_method[i]
            }
          })   
          crit=c(input$enable_forward, input$enable_lasso, input$enable_ridge,input$enable_scad)
          sel_param=lapply(sel_method[crit],
                           function(sel){
                             if(sel!="fwd_method"){
                             get(sel)()[["criteria"]][2,2]
                             }else{
                               NA
                             }
                             })
          sel_param_value=do.call(c,sel_param)
          criterion <- c(input$fwd_criteria,rep("RMSE",3))
          criterion <- criterion[crit]
          rc <- data.frame(Method = method_cr()[,1],Metric=criterion,
                           Value = NA)
          rc$Value <- ifelse(rc$Method == "Forward", round(fwd_method()$criteria[1,2],2),rc$Value)
          rc$Value <- ifelse(rc$Method == "LASSO", round(lasso_method()$criteria[2,2],2),rc$Value)
          rc$Value <- ifelse(rc$Method == "SCAD", round(scad_method()$criteria[2,2],2),rc$Value)
          rc$Value <- ifelse(rc$Method == "Ridge", round(ridge_method()$criteria[2,2],2),rc$Value)
      ...
      output$result_parameters <- renderDataTable({
        if(n>0){
          is_user=c(F,input$lasso_criteria=="User",
                    input$ridge_criteria=="User",
                    input$scad_criteria=="User")
        ...
        sel_crit=c(input$enable_forward, input$enable_lasso, input$enable_ridge,input$enable_scad)
        sel_param=lapply(sel_method[sel_crit],
                         function(sel){
                           if(sel!="fwd_method"){
                             get(sel)()[["criteria"]][1,2]
                           }else{
                             NA
                           }
                         })
        sel_param_value=do.call(c,sel_param)
        rparam <- data.frame(Method = method_cr()[,1],
                             Parameter=round(sel_param_value,2))
        datatable(rparam, 
                  escape = FALSE, 
                  rownames = FALSE,
                  options = list(pageLength = 5,
